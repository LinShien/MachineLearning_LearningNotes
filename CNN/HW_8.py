# -*- coding: utf-8 -*-
"""
Created on Wed May  2 00:23:35 2018
...
from keras.layers import  LeakyReLU, PReLU, ELU, Dense, Dropout, Flatten, BatchNormalization, Activation
from keras.utils import np_utils


np.random.seed(1337)  # for reproducibility
batch_size = 200   #一次訓練的data個數
nb_classes = 10     #判斷的種類
...
X_trainning = np.reshape(X_trainning, (60000, 28**2)).astype('float32')  /255     #換成60000x764 且為float的矩陣, 並轉換精確度
X_testing = np.reshape(X_testing, (10000, 28**2)).astype('float32') /255         #換成10000x764 且為float的矩陣, 並轉換精確度

#keras.utils.to_categorical(y, num_classes=None) => Converts a class vector (integers) to binary class matrix.
Y_trainning = np_utils.to_categorical(Y_trainning, nb_classes)        #把Y換成60000x(10x1)的vectors
...
model.add(Dense(input_dim = 28**2, units = 500))    #hidden units = 500
#model.add(BatchNormalization())
model.add(PReLU())
#model.add(Dropout(0.5))
model.add(Dense(units=500))                #第2層hidden layer
#model.add(BatchNormalization())
model.add(PReLU())
